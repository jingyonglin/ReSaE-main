...
class RESAEConvEStatement(ReSaEEncoder):
    def __init__(self, kg_graph_repr: Dict[str, np.ndarray], config: dict):
        ...
        self.n_filters = config['RESAEARGS']['N_FILTERS']
        self.kernel_sz = config['RESAEARGS']['KERNEL_SZ']
        # self.bias = config['RESAEARGS']['BIAS']
        self.k_w = config['RESAEARGS']['K_W']
        self.k_h = config['RESAEARGS']['K_H']
        ...
        self.m_conv1 = torch.nn.Conv2d(1, out_channels=self.n_filters,
                                       ...
                                       padding=0, bias=config['RESAEARGS']['BIAS'])
...
class ReSaE_ConvKB_Statement(ReSaEEncoder):
    model_name = 'RESAE_ConvKB_Statement'

    def __init__(self, kg_graph_repr: Dict[str, np.ndarray], config: dict):
        super(self.__class__, self).__init__(kg_graph_repr, config)

        self.model_name = 'RESAE_ConvKB_Statement'
        self.hid_drop2 = config['RESAEARGS']['HID_DROP2']
        self.feat_drop = config['RESAEARGS']['FEAT_DROP']
        self.n_filters = config['RESAEARGS']['N_FILTERS']
        self.kernel_sz = config['RESAEARGS']['KERNEL_SZ']
        ...
        self.pooling = config['RESAEARGS']['POOLING']
...
class ReSaE_Transformer_Triples(ReSaEEncoder):
    model_name = 'ReSaE_Transformer_Triples'

    def __init__(self, kg_graph_repr: Dict[str, np.ndarray], config: dict):

        super(self.__class__, self).__init__(kg_graph_repr, config)

        self.model_name = 'ReSaE_Transformer_Triples'
        self.hid_drop2 = config['RESAEARGS']['HID_DROP2']
        self.feat_drop = config['RESAEARGS']['FEAT_DROP']
        ...
        encoder_layers = TransformerEncoderLayer(self.d_model, self.num_heads, self.num_hidden, config['RESAEARGS']['HID_DROP2'])
...
class Transformer_Baseline(ReSaEBase):
