...
from .gnn_layer import ReSaEConvLayer

class ReSaEBase(torch.nn.Module):
    def __init__(self, config):
        super(ReSaEBase, self).__init__()
        """ Not saving the config dict bc model saving can get a little hairy. """

        self.act = torch.tanh if 'ACT' not in config['RESAEARGS'].keys() \
            else config['RESAEARGS']['ACT']
        self.bceloss = torch.nn.BCELoss()
        self.mseloss = torch.nn.MSELoss()
        self.emb_dim = config['EMBEDDING_DIM']
        self.num_rel = config['NUM_RELATIONS']
        self.num_ent = config['NUM_ENTITIES']
        self.n_bases = config['RESAEARGS']['N_BASES']
        self.n_layer = config['RESAEARGS']['LAYERS']
        self.gcn_dim = config['RESAEARGS']['GCN_DIM']
        self.hid_drop = config['RESAEARGS']['HID_DROP']
        # self.bias = config['RESAEARGS']['BIAS']
        self.model_nm = config['MODEL_NAME'].lower()
        self.triple_mode = config['STATEMENT_LEN'] == 3
        self.qual_mode = config['RESAEARGS']['QUAL_REPR']
        self.mse_num = config['RESAEARGS']['MSE']
        self.klloss = nn.KLDivLoss(reduction="batchmean")
        self.kl_num = config['RESAEARGS']['KL']

    def loss(self, pred, true_label):
        res = self.bceloss(pred, true_label)
        return res


class ReSaEEncoder(ReSaEBase):
    def __init__(self, graph_repr: Dict[str, np.ndarray], config: dict, timestamps: dict = None):
        ...
        elif config['RESAEARGS']['OPN'] == 'rotate' or config['RESAEARGS']['QUAL_OPN'] == 'rotate':
        ...
        self.conv1 = ReSaEConvLayer(self.emb_dim, self.gcn_dim, self.num_rel, act=self.act,
                                       config=config)
        self.conv2 = ReSaEConvLayer(self.gcn_dim, self.emb_dim, self.num_rel, act=self.act,
    ...
    def forward_ease(self, sub, rel, drop1, drop2,
                     quals=None, embed_qualifiers: bool = False, return_mask: bool = False):
        """"
        :param sub:
        :param rel:
        ...
        x, r = self.conv1(x=x, edge_index=self.edge_index,
                          edge_type=self.edge_type, rel_embed=r,
                          qualifier_ent=None,
                          qualifier_rel=None,
                          quals=self.quals)
        ...
        if embed_qualifiers:
            ...
            if not return_mask:
                ...
                # and put True where qual entities and relations are actually padding index 0
                mask[:, 2:] = quals == 0
                return sub_emb, rel_emb, qual_obj_emb, qual_rel_emb, x,r, mask
